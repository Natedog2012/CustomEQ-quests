...
sub DiaWind { 
	...
	if(!$nobracket){
		...
		if(!$data[1] && $data[0] && $Opt == 1) { $Button0 = ucfirst($data[0]); }
	}
	...
	if($Timer > 0){ $ClickQueue = "<c \"#F07F00\">This message will disappear in " . $Timer . " second(s)...</c>"; }
		...
		if($Opt == 1 && !$data[1] && $data[0]){ $ClickQueue = "<c \"#F07F00\">Click '$data[0]' to continue'...</c>";  }
	...
	else{ $client->Popup2($Title,  "<c \"#CCFF33\">" . $Speaking . "</c><br><br>" . $quo . "" . $Text . "" . $quo . "" . "<br><br>" . $ClickQueue . "", $PopId, $PopId2, $Opt, $Timer, $Button0, $Button1); }
